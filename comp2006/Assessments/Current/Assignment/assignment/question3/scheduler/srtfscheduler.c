...
void* SrtfSchedulerThread(void* arg) {
    ...
    while (memoryPtr->buffer1IsFull == FALSE) {
        pthread_cond_wait(&memoryPtr->buffer1IsFullCond, &memoryPtr->buffer1Lock);
    }
    pthread_mutex_unlock(&memoryPtr->buffer1Lock);

    /* Block until other child thread completes execution, then begin execution */
    pthread_mutex_lock(&memoryPtr->executionLock);
    while (!memoryPtr->PpSchedulerIsFinished) {
        pthread_cond_wait(&memoryPtr->PpSchedulerIsFinishedCond, &memoryPtr->executionLock);
    }
    ...
    /* Read input file */
        ...
        if (numProcesses > 0) {
            while (numProcessesCompleted != numProcesses) {
                /* Denotes index of process with shortest time remaining */
                index = -1;            
                /* Denotes time of process with shortest time remaining */
                time = 2147483647;     
                ...
                if (index != -1) {
                    ...
                    /* Consider putting into some data structure rather than printing */
