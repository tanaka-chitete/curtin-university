    ...
    private static void readAlbumFile(File albumFile, Album album) throws IOException
    {
        ...
        while(line != null)
        {
            if(line.trim().length() > 0) // Ignore blank lines
            {
                ...
                String imageName = albumFile.getParent() + File.separatorChar + parts[0];
                String caption = String.format("caption=%s ", parts[1]);
                Entry image = new BasicEntry(imageName, caption);

                if(parts.length > 2)
                {
                    image = new Metadata(image);
                    String separator = "";
                    for (int i = 2; i < parts.length; i++) {
                        image.addToCaption(separator);
                        image.addToCaption(String.format("%s ", parts[i]));
                        separator = " ";
                    }
                }
